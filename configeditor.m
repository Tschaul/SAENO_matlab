function varargout = configeditor(varargin)
% CONFIGEDITOR MATLAB code for configeditor.fig
%      CONFIGEDITOR, by itself, creates a new CONFIGEDITOR or raises the existing
%      singleton*.
%
%      H = CONFIGEDITOR returns the handle to a new CONFIGEDITOR or the handle to
%      the existing singleton*.
%
%      CONFIGEDITOR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CONFIGEDITOR.M with the given input arguments.
%
%      CONFIGEDITOR('Property','Value',...) creates a new CONFIGEDITOR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before configeditor_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to configeditor_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help configeditor

% Last Modified by GUIDE v2.5 26-Jun-2013 09:38:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @configeditor_OpeningFcn, ...
                   'gui_OutputFcn',  @configeditor_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before configeditor is made visible.
function configeditor_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to configeditor (see VARARGIN)


% Choose default command line output for configeditor
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes configeditor wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% SET UP WORKFLOWIMAGES

flist=dir('guimedia/configeditor/*.png');

handles.workflowimages=struct;

for i=1:numel(flist)
    
    im=imread(['guimedia/configeditor/' flist(i).name]);
...
handles.defaults=loadConfigFile('defaults.txt');
handles.modules=loadConfigFile('modules.txt');
...
if numel(varargin)>0
    ...
    handles.config=loadConfigFile('defaults.txt');
...
function varargout = configeditor_OutputFcn(hObject, eventdata, handles) 
